...
def main(input_net_name):
    ...
    images_dir = config.WIDER_FACE_IMG_DIR  # source wider_face data file directory
    anno_file = config.WIDER_FACE_ANNO_FILE
    out_dir = config.GAN_DATA_ROOT_DIR      # output directory

    target_size = config.NET_SIZE[net_name]     # size of generated target image
    save_dir = '{}/{}'.format(out_dir, net_name)

    pos_save_dir = save_dir + '/positive'
    part_save_dir = save_dir + '/part'
    neg_save_dir = save_dir + '/negative'
    if not os.path.exists(save_dir):        # build output directories
        os.mkdir(save_dir)
    ...
    f1 = open(os.path.join(save_dir, 'pos_' + str(target_size) + '.txt'), 'a')  # build and open new annotation files
    ...
    n_lines = len(annotations)          # num of lines in annotation text file
    print('%d pics in total' % n_lines)
    p_idx = 0  # positive samples index
    n_idx = 0  # negative samples index
    d_idx = 0  # part samples index
    idx = 0     # index of raw images processed
    box_idx = 0
    
    idx_line = 0    # index of lines of image names (head of group of lines for each image)
    while idx_line < n_lines:
        ...
        for i in range(n_faces):
            ...
            box = utils.convert_bbox((x1, y1, w, h), False)     # convert from x1, y1, w, h to x1, y1, x2, y2
            bboxes.append(box)
        ...
        # 随机生成 negative 样本(Background)，
        neg_num = 0
        while neg_num < BACKGRAND_NEG_NUM:
            size = npr.randint(target_size, min(width, height) / 2)  # min size: target face size, maxsize: half of whole pic size
            ...
            cropped_im = img[ny: ny + size, nx: nx + size, :]   # crop and resize sample
        ...
        for box in bboxes:
            ...
            if max(w, h) < 40 or x1 < 0 or y1 < 0:  # skip invalid or less effective b-boxes
                continue
            ...
            for i in range(POS_PART_NUM):
                ...
                offset_x1 = (x1 - nx1) / float(size)        # offset is in range [-1, 1]
